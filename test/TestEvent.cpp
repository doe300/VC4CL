...
TestEvent::TestEvent() : event_count(0), context(NULL), queue(NULL), user_event(NULL)
{
...
bool TestEvent::setup()
{
    ...
    context = VC4CL_FUNC(clCreateContext)(NULL, 1, &device_id, NULL, NULL, &state);
...
void TestEvent::testGetEventInfo()
{
    ...
    TEST_ASSERT_EQUALS(nullptr, *(cl_command_queue*)buffer);
    ...
    TEST_ASSERT_EQUALS(CL_COMMAND_USER, *(cl_command_type*)buffer);
    ...
    TEST_ASSERT_EQUALS(-42, *(cl_int*)buffer);
...
{
    cl_event event = NULL;
    cl_int state = VC4CL_FUNC(clEnqueueBarrierWithWaitList)(queue, 0, NULL, &event);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
...
{
    cl_event event = NULL;
    cl_int state = VC4CL_FUNC(clEnqueueMarkerWithWaitList)(queue, 0, NULL, &event);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT(event != NULL);
...
{
    TEST_ASSERT_EQUALS(1, toType<Event>(user_event)->getReferences());
    cl_int state = VC4CL_FUNC(clRetainEvent)(user_event);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    
    TEST_ASSERT_EQUALS(2, toType<Event>(user_event)->getReferences());
...
void TestEvent::testReleaseEvent()
{
    ...
    TEST_ASSERT_EQUALS(1, event_count);
}
