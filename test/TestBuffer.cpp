...
{
    cl_int errcode = CL_SUCCESS;
    buffer = VC4CL_FUNC(clCreateBuffer)(context, CL_MEM_READ_ONLY|CL_MEM_WRITE_ONLY|CL_MEM_USE_HOST_PTR, 1024, NULL, &errcode);
    TEST_ASSERT(errcode != CL_SUCCESS);
    TEST_ASSERT_EQUALS(nullptr, buffer);
    
    buffer = VC4CL_FUNC(clCreateBuffer)(context, 0, 1024, NULL, &errcode);
...
{
    cl_int errcode = CL_SUCCESS;
    cl_mem sub_buffer = VC4CL_FUNC(clCreateSubBuffer)(buffer, CL_MEM_WRITE_ONLY|CL_MEM_READ_ONLY, 0, NULL, &errcode);
    TEST_ASSERT(errcode != CL_SUCCESS);
...
{
    cl_event event = NULL;
    cl_int state = VC4CL_FUNC(clEnqueueReadBuffer)(queue, buffer, CL_FALSE, 0, 256, NULL, 0, NULL, &event);
    TEST_ASSERT(state != CL_SUCCESS);
    TEST_ASSERT_EQUALS(nullptr, event);
    
    char tmp[1024];
    state = VC4CL_FUNC(clEnqueueReadBuffer)(queue, buffer, CL_TRUE, 256, 512, tmp, 0, NULL, &event);
...
{
    cl_event event = NULL;
    cl_int state = VC4CL_FUNC(clEnqueueWriteBuffer)(queue, buffer, CL_FALSE, 0, 256, NULL, 0, NULL, &event);
    TEST_ASSERT(state != CL_SUCCESS);
    TEST_ASSERT_EQUALS(nullptr, event);
    
    char tmp[1024];
    state = VC4CL_FUNC(clEnqueueWriteBuffer)(queue, buffer, CL_TRUE, 256, 512, tmp, 0, NULL, &event);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT(event != NULL);
    TEST_ASSERT_EQUALS(CL_COMPLETE, toType<Event>(event)->getStatus());
...
{
    cl_event event = NULL;
    cl_int state = VC4CL_FUNC(clEnqueueFillBuffer)(queue, buffer, NULL, 17, 0, 256, 0, NULL, &event);
    TEST_ASSERT(state != CL_SUCCESS);
    TEST_ASSERT_EQUALS(nullptr, event);
    
    char tmp[1024];
    memset(tmp, 0x55, 512);
    state = VC4CL_FUNC(clEnqueueFillBuffer)(queue, buffer, tmp, 8, 256, 512, 0, NULL, &event);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT(event != NULL);
...
void TestBuffer::testEnqueueMapBuffer()
{
    cl_int errcode = CL_SUCCESS;
    cl_event event = NULL;
    mapped_ptr = VC4CL_FUNC(clEnqueueMapBuffer)(queue, buffer, CL_TRUE, 0, 256, 1512, 0, NULL, &event, &errcode);
    TEST_ASSERT(errcode != CL_SUCCESS);
    TEST_ASSERT_EQUALS(nullptr, mapped_ptr);
    TEST_ASSERT_EQUALS(nullptr, event);
    
    mapped_ptr = VC4CL_FUNC(clEnqueueMapBuffer)(queue, buffer, CL_TRUE, 0, 256, 512, 0, NULL, &event, &errcode);
    TEST_ASSERT_EQUALS(CL_SUCCESS, errcode);
...
{
    cl_event event = NULL;
    cl_int state = VC4CL_FUNC(clEnqueueUnmapMemObject)(queue, buffer, mapped_ptr, 0, NULL, &event);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT(event != NULL);
    ...
    event = NULL;
    state = VC4CL_FUNC(clEnqueueUnmapMemObject)(queue, buffer, mapped_ptr, 0, NULL, &event);
    TEST_ASSERT(state != CL_SUCCESS);
    TEST_ASSERT_EQUALS(nullptr, event);
