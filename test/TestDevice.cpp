...
void TestDevice::testGetDeviceInfo()
{
    ...
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT_EQUALS(sizeof(cl_bool), info_size);
    TEST_ASSERT_EQUALS(HAS_COMPILER, *(cl_bool*)buffer);
    ...
    TEST_ASSERT_EQUALS(0, *(cl_device_fp_config*)buffer);
    ...
    TEST_ASSERT_EQUALS(CL_EXEC_KERNEL, *(cl_device_exec_capabilities*)buffer);
    ...
    TEST_ASSERT_EQUALS(CL_READ_WRITE_CACHE, *(cl_device_mem_cache_type*)buffer);
    ...
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT_EQUALS(sizeof(cl_ulong), info_size);
    TEST_ASSERT((*(cl_ulong*)buffer) >= 32 * 1024);
    ...
    TEST_ASSERT_EQUALS(CL_GLOBAL, *(cl_device_local_mem_type*)buffer);
    
    state = VC4CL_FUNC(clGetDeviceInfo)(device, CL_DEVICE_MAX_CLOCK_FREQUENCY, 1024, buffer, &info_size);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT_EQUALS(sizeof(cl_uint), info_size);
    TEST_ASSERT_EQUALS(250, *(cl_uint*)buffer);
    ...
    TEST_ASSERT(((size_t*)buffer)[0] >= 1);
    TEST_ASSERT(((size_t*)buffer)[1] >= 1);
    TEST_ASSERT(((size_t*)buffer)[2] >= 1);
    
    state = VC4CL_FUNC(clGetDeviceInfo)(device, CL_DEVICE_MEM_BASE_ADDR_ALIGN, 1024, buffer, &info_size);
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT_EQUALS(sizeof(cl_uint), info_size);
    TEST_ASSERT_EQUALS(sizeof(cl_int16) * 8 /* size in bits */, *(cl_uint*)buffer);
    ...
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT_EQUALS(sizeof(cl_uint), info_size);
    TEST_ASSERT_EQUALS(0, *(cl_uint*)buffer);
    ...
    TEST_ASSERT_EQUALS(nullptr, *(cl_device_id*)buffer);
    ...
    TEST_ASSERT_EQUALS(0, *(cl_device_partition_property*)buffer);
    ...
    TEST_ASSERT_EQUALS(0, *(cl_device_affinity_domain*)buffer);
    ...
    TEST_ASSERT_EQUALS(Platform::getVC4CLPlatform().toBase(), *(cl_platform_id*)buffer);
    ...
    TEST_ASSERT_EQUALS(CL_SUCCESS, state);
    TEST_ASSERT_EQUALS(sizeof(size_t), info_size);
    TEST_ASSERT_EQUALS(0, *(size_t*)buffer);
    ...
    TEST_ASSERT_EQUALS(CL_DEVICE_TYPE_GPU, *(cl_device_type*)buffer);
...
void TestDevice::testCreateSubDevice()
{
    ...
    TEST_ASSERT_EQUALS(0, num_devices);
}
