...
namespace vc4cl
{
    ...
    struct TextureType
    {
        constexpr TextureType(
            unsigned char val, unsigned char bpp, unsigned char chans, bool isRasterFormat = false) noexcept :
            id(val),
            bitsPerPixel(bpp), channels(chans), isRasterFormat(isRasterFormat)
        {
        }
    ...
    public:
         ...
         * "Cache Swizzle"
         */
        // TODO is the cache swizzle the value used in
        // https://github.com/anholt/mesa/blob/master/src/gallium/drivers/vc4/vc4_formats.c#L73 to re-order the
        // channels?
    ...
    /*
     * third UNIFORM-value read by the TMU to configure texture-reads. This value is only read for Cube map strides or
     * child image parameters
     *
     * See Broadcom specification, table 17
     */
