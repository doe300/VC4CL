...
cl_int CommandQueue::getInfo(cl_command_queue_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret) const
{
	...
	switch(param_name)
	{
		...
		case CL_QUEUE_DEVICE:
			...
			return returnValue<cl_device_id>(const_cast<cl_device_id>(context()->device->toBase()), param_value_size, param_value, param_value_size_ret);
		...
		case CL_QUEUE_PROPERTIES:
			...
			return returnValue<cl_command_queue_properties>(properties, param_value_size, param_value, param_value_size_ret);
	}

	return returnError(CL_INVALID_VALUE, __FILE__, __LINE__, buildString("Invalid cl_command_queue_info value %u", param_name));
}
...
cl_int CommandQueue::finish()
{
	...
	while((event = peekQueue(this)) != nullptr)
		ignoreReturnValue(event->waitFor(), __FILE__, __LINE__, "This method does not check the states of the single events as per specification");
 ...
 *
 *  \param device must be a device associated with context.  It can either be in the list of devices specified when context is created using clCreateContext
 *  or have the same device type as device type specified when context is created using clCreateContextFromType.
 *
 *  \param properties specifies a list of properties for the command-queue.  This is a bit-field and is described in table 5.1.
 *  Only command-queue properties specified in table 5.1 can be set in properties; otherwise the value specified in properties is considered to be not valid.
 *
 *  \param errcode_ret will return an appropriate error code.  If errcode_ret is NULL, no error code is returned.
 *
 *  \return clCreateCommandQueue returns a valid non-zero command-queue and errcode_ret is set to CL_SUCCESS if the command-queue is created successfully.
 ...
 */
cl_command_queue VC4CL_FUNC(clCreateCommandQueue)(cl_context context, cl_device_id device, cl_command_queue_properties properties, cl_int* errcode_ret)
{
	...
	if(toType<Context>(context)->device != toType<Device>(device))
		return returnError<cl_command_queue>(CL_INVALID_DEVICE, errcode_ret, __FILE__, __LINE__, "Device of context does not match given device!");
 ...
 *
 *  \return clRetainCommandQueue returns CL_SUCCESS if the function is executed successfully.  Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  clCreateCommandQueue performs an implicit retain.  This is very helpful for 3rd party libraries, which typically get a command-queue passed to them by the application.
 *  However, it is possible that the application may delete the command-queue without informing the library. Allowing functions to attach to (i.e. retain) and release a
 ...
 *
 *  \return clReleaseCommandQueue returns CL_SUCCESS if the function is executed successfully.  Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  After the command_queue reference count becomes zero and all commands queued to command_queue have finished (eg. kernel executions, memory object updates etc.),
 ...
 *
 *  \param param_value_size is used to specify the size in bytes of memory pointed to by param_value. This size must be >= size of return type as described in table 5.2.
 ...
 *
 *  \return clGetCommandQueueInfo returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
 *  - CL_INVALID_VALUE if param_name is not one of the supported values or if size in bytes specified by param_value_size is < size of return type as specified in table 5.2
 *  and param_value is not a NULL value.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
cl_int VC4CL_FUNC(clGetCommandQueueInfo)(cl_command_queue command_queue, cl_command_queue_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret)
{
	CHECK_COMMAND_QUEUE(toType<CommandQueue>(command_queue))
	return toType<CommandQueue>(command_queue)->getInfo(param_name, param_value_size, param_value, param_value_size_ret);
}
 ...
 *   Only command-queue properties specified in table 5.1 can be set in properties; otherwise the value specified in properties is considered to be not valid.
 *
 *  \param enable determines whether the values specified by properties are enabled (if enable is CL_TRUE) or disabled (if enable is CL_FALSE) for the command-queue.
 *  The property values are described in table 5.1.
 *
 *  \param old_properties returns the command-queue properties before they were changed by clSetCommandQueueProperty.  If old_properties is NULL, it is ignored.
 *  As specified in table 5.1, the CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE command-queue property determines whether the commands in a command-queue are executed in-order or out-of-order.
 *  Changing this command-queue property will cause the OpenCL implementation to block until all previously queued commands in command_queue have completed.
 *  This can be an expensive operation and therefore changes to the CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE property should be only done when absolutely necessary.
 *
 *  \return clSetCommandQueueProperty returns CL_SUCCESS if the command-queue properties are successfully updated.  It returns
 ...
 */
cl_int VC4CL_FUNC(clSetCommandQueueProperty)(cl_command_queue command_queue, cl_command_queue_properties properties, cl_bool enable, cl_command_queue_properties* old_properties)
{
	CHECK_COMMAND_QUEUE(toType<CommandQueue>(command_queue))
	if(old_properties != nullptr)
	{
		cl_int status = toType<CommandQueue>(command_queue)->getInfo(CL_QUEUE_PROPERTIES, sizeof(cl_command_queue_properties), old_properties, nullptr);
		if(status != CL_SUCCESS)
			return status;
	}
 ...
 *
 *  Issues all previously queued OpenCL commands in command_queue to the device associated with command_queue. clFlush only guarantees that all queued commands to command_queue will eventually be submitted to the appropriate device.
 *  There is no guarantee that they will be complete after clFlush returns.
 *
 *  \return clFlush returns CL_SUCCESS if the function call was executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  Any blocking commands queued in a command-queue and clReleaseCommandQueue perform an implicit flush of the command-queue. These blocking commands are clEnqueueReadBuffer,
 *  clEnqueueReadBufferRect, clEnqueueReadImage, with blocking_read set to CL_TRUE ; clEnqueueWriteBuffer, clEnqueueWriteBufferRect, clEnqueueWriteImage with blocking_write set to CL_TRUE ;
 *  clEnqueueMapBuffer, clEnqueueMapImage with blocking_map set to CL_TRUE ; or clWaitForEvents.
 *
 ...
 *  clFlush or any blocking commands that perform an implicit flush of the command-queue where the commands that refer to these event objects are enqueued.
 */
 ...
 *
 *  \return clFinish returns CL_SUCCESS if the function call was executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
