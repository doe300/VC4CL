...
cl_int Device::getInfo(cl_device_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret) const
{
	...
	switch(param_name)
	{
		...
		case CL_DEVICE_MAX_MEM_ALLOC_SIZE:
			//"Max size of memory object allocation in bytes.  The minimum value is max (1/4th of CL_DEVICE_GLOBAL_MEM_SIZE, 1 MB)"
		...
		case CL_DEVICE_EXECUTION_CAPABILITIES:
			...
			return returnValue<cl_device_exec_capabilities>(CL_EXEC_KERNEL, param_value_size, param_value, param_value_size_ret);
		case CL_DEVICE_QUEUE_PROPERTIES:
			...
			return returnValue<cl_command_queue_properties>(CL_QUEUE_PROFILING_ENABLE, param_value_size, param_value, param_value_size_ret);
		...
		case CL_DEVICE_PRINTF_BUFFER_SIZE:
			...
			//TODO printf support (OpenCL 1.2, page 284) -> write to VPM (special area, with index for current position) and print from host..
		...
		case CL_DEVICE_PARTITION_PROPERTIES:
			...
			return returnValue(&dummy, sizeof(cl_device_partition_property), 1, param_value_size, param_value, param_value_size_ret);
		...
		case CL_DEVICE_SPIR_VERSIONS:
			...
			//"A space separated list of SPIR versions supported by the device. For example returning “1.2 2.0” in this query implies that SPIR version 1.2 and 2.0 are supported by the implementation."
			//both supported LLVMs support SPIR 1.2
		...
		case CL_DEVICE_IL_VERSION_KHR:
			...
			return returnString(SPIRV_VERSION " " SPIR_PREFIX SPIR_VERSION, param_value_size, param_value, param_value_size_ret);
		...
		default:
			...
			return returnError(CL_INVALID_VALUE, __FILE__, __LINE__, buildString("Invalid cl_device_info value %u", param_name));
	}
 ...
 *
 *  \param device_type is a bitfield that identifies the type of OpenCL device.  The device_type can be used to query specific OpenCL devices or all OpenCL devices available.
 *  The valid values for device_type are specified in table 4.2.
 *
 *  \param num_entries is the number of cl_device_id entries that can be added to devices. If devices is not NULL, the num_entries must be greater than zero.
 *
 *  \param devices returns a list of OpenCL devices found.  The cl_device_id values returned in devices can be used to identify a specific OpenCL device.
 *  If devices argument is NULL, this argument is ignored.  The number of OpenCL devices returned is the minimum of the value specified by num_entries or
 *  the number of OpenCL devices whose type matches device_type.
 *
 *  \param num_devices returns the number of OpenCL devices available that match device_type.  If num_devices is NULL, this argument is ignored.
 *
 *  \return clGetDeviceIDs returns CL_SUCCESS if the function is executed successfully.  Otherwise, it returns one of the following errors:
 ...
 */
cl_int VC4CL_FUNC(clGetDeviceIDs)(cl_platform_id platform, cl_device_type device_type, cl_uint num_entries, cl_device_id* devices, cl_uint* num_devices)
{
 ...
 *  Gets specific information about an OpenCL device. device may be a device returned by clGetDeviceIDs or a sub-device created by clCreateSubDevices.
 *  If device is a sub-device, the specific information for the sub-device will be returned. The information that can be queried using clGetDeviceInfo is specified in table 4.3.
 *
 *  \param device is a device returned by clGetDeviceIDs.
 *
 *  \param param_name is an enumeration constant that identifies the device information being queried.  It can be one of the following values as specified in table 4.3.
 *
 *  \param param_value is a pointer to memory location where appropriate values for a given param_name as specified in table 4.3 will be returned.  If param_value is NULL, it is ignored.
 *
 *  \param param_value_size specifies the size in bytes of memory pointed to by param_value.  This size in bytes must be >= size of return type specified in table 4.3.
 *
 ...
 *
 *  The device queries described in table 4.3 should return the same information for a root-level device i.e. a device returned by clGetDeviceIDs and any sub-devices
 *  created from this device except for the following queries:
 *  CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, CL_DEVICE_BUILT_IN_KERNELS, CL_DEVICE_PARENT_DEVICE, CL_DEVICE_PARTITION_TYPE, CL_DEVICE_REFERENCE_COUNT
 *
 *  \return clGetDeviceInfo returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_DEVICE if device is not valid.
 *  - CL_INVALID_VALUE if param_name is not one of the supported values or if size in bytes specified by param_value_size is < size of return type as specified in table 4.3
 *  and param_value is not a NULL value or if param_name is a value that is available as an extension and the corresponding extension is not supported by the device.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 */
cl_int VC4CL_FUNC(clGetDeviceInfo)(cl_device_id device, cl_device_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret)
{
 ...
 *  The output sub-devices may be used in every way that the root (or parent) device can be used, including creating contexts, building programs, further calls to clCreateSubDevices
 *  and creating command-queues.  When a command-queue is created against a sub-device, the commands enqueued on the queue are executed only on the sub-device.
 *
 *  \param in_device is the device to be partitioned.
 *
 *  \param properties specifies how in_device is to be partition described by a partition name and its corresponding value. Each partition name is immediately followed
 *  by the corresponding desired value. The list is terminated with 0. The list of supported partitioning schemes is described in table 4.4.
 ...
 *
 *  \param out_devices is the buffer where the OpenCL sub-devices will be returned. If out_devices is NULL, this argument is ignored. If out_devices is not NULL,
 ...
 *
 *  \param num_devices_ret returns the number of sub-devices that device may be partitioned into according to the partitioning scheme specified in properties.
 *  If num_devices_ret is NULL, it is ignored.
 *
 *  \return clCreateSubDevices returns CL_SUCCESS if the partition is created successfully. Otherwise, it returns a NULL value with the following error values returned in errcode_ret:
 ...
 *  - CL_INVALID_DEVICE_PARTITION_COUNT if the partition name specified in properties is CL_DEVICE_PARTITION_BY_COUNTS and the number of sub-devices requested
 *  exceeds CL_DEVICE_PARTITION_MAX_SUB_DEVICES or the total number of compute units requested exceeds CL_DEVICE_PARTITION_MAX_COMPUTE_UNITS for in_device,
 *  or the number of compute units requested for one or more sub-devices is less than zero or the number of sub-devices requested exceeds CL_DEVICE_PARTITION_MAX_COMPUTE_UNITS for in_device.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
cl_int VC4CL_FUNC(clCreateSubDevices)(cl_device_id in_device, const cl_device_partition_property* properties, cl_uint num_devices, cl_device_id *out_devices, cl_uint* num_devices_ret)
{
 ...
 *
 *  \return clRetainDevice returns CL_SUCCESS if the function is executed successfully or the device is a root-level device. Otherwise, it returns one of the following errors:
 ...
 *  If device is a root level device i.e. a cl_device_id returned by clGetDeviceIDs, the device reference count remains unchanged.
 *
 *  \return clReleaseDevice returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_DEVICE if device is not a valid sub-device created by a call to clCreateSubDevices.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
