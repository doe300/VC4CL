...
namespace vc4cl
{
	...
	template<typename T>
	CHECK_RETURN inline T returnError(cl_int error, cl_int* errcode_ret, const std::string& file, unsigned line, const std::string& reason)
	{
#ifdef DEBUG_MODE
	...
	CHECK_RETURN inline cl_int returnError(cl_int error, const std::string& file, unsigned line, const std::string& reason)
	{
#ifdef DEBUG_MODE
		std::cout << "[VC4CL] Error in '" << file << ":" << line << "', returning status " << error << ":" << reason << std::endl;
#endif
	...
	inline void ignoreReturnValue(cl_int state, const std::string& file, unsigned line, const std::string& reasonForIgnoring)
	{
