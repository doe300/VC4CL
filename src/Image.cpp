...
static const std::unordered_map<cl_image_format, TextureType, vc4cl::hash_cl_image_format, equal_cl_image_format> supportedFormats = {
	...
	{cl_image_format{CL_RGBA, CL_UNORM_INT8}, RGBA32R},
	{cl_image_format{CL_RGBA, CL_UNORM_INT16}, S16},	//XXX
	{cl_image_format{CL_RGBA, CL_SIGNED_INT8}, RGBA32R},
	{cl_image_format{CL_RGBA, CL_SIGNED_INT16}, S16},	//XXX
	...
	{cl_image_format{CL_RGBA, CL_HALF_FLOAT}, RGBA64},
	{cl_image_format{CL_RGBA, CL_FLOAT}, A1}, //XXX
	//XXX { CL_BGRA, CL_UNORM_INT8} is also required by FULL_PROFILE, not EMBEDDED_PROFILE, but OpenCL-CTS requires it for image-tests
...
cl_int Image::enqueueRead(CommandQueue* commandQueue, cl_bool blockingRead, const size_t* origin, const size_t* region, size_t row_pitch, size_t slice_pitch, void* ptr, cl_uint numEventsInWaitList, const cl_event* waitList, cl_event* event)
{
	if(ptr == nullptr)
...
cl_int Image::enqueueCopyInto(CommandQueue* commandQueue, Image* destination, const size_t* srcOrigin, const size_t* dstOrigin, const size_t* region, cl_uint numEventsInWaitList, const cl_event* waitList, cl_event* event)
{
	if(context() != destination->context())
		return returnError(CL_INVALID_CONTEXT, __FILE__, __LINE__, "Context of source and destination images do not match!");

	if(channelOrder.id != destination->channelOrder.id || channelType.id != destination->channelType.id)
		return returnError(CL_INVALID_CONTEXT, __FILE__, __LINE__, "Channel formats  of source and destination images do not match!");
	...
	if(destination == this)
	{
		bool xOverlaps = (srcOrigin[0] <= dstOrigin[0] && srcOrigin[0] + region[0] > dstOrigin[0]) || (dstOrigin[0] <= srcOrigin[0] && dstOrigin[0] + region[0] > srcOrigin[0]);
		bool yOverlaps = false;
		if(imageType.numDimensions > 1 || imageType.isImageArray)
				yOverlaps = (srcOrigin[1] <= dstOrigin[1] && srcOrigin[1] + region[1] > dstOrigin[1]) || (dstOrigin[1] <= srcOrigin[1] && dstOrigin[1] + region[1] > srcOrigin[1]);
		bool zOverlaps = false;
		if(imageType.numDimensions > 2 || (imageType.numDimensions == 2 && imageType.isImageArray))
			zOverlaps = (srcOrigin[2] <= dstOrigin[2] && srcOrigin[2] + region[2] > dstOrigin[2]) || (dstOrigin[2] <= srcOrigin[2] && dstOrigin[2] + region[2] > srcOrigin[2]);
...
cl_int Image::enqueueCopyFromToBuffer(CommandQueue* commandQueue, Buffer* buffer, const size_t* origin, const size_t* region, const size_t bufferOffset, bool copyIntoImage, cl_uint numEventsInWaitList, const cl_event* waitList, cl_event* event)
{
	...
	if(deviceBuffer.get() && buffer->deviceBuffer.get())
		return returnError(CL_INVALID_MEM_OBJECT, __FILE__, __LINE__, "Cannot copy between image and buffer using the same data!");
...
void* Image::enqueueMap(CommandQueue* commandQueue, cl_bool blockingMap, cl_map_flags mapFlags, const size_t* origin, const size_t* region, size_t* rowPitchOutput, size_t* slicePitchOutput, cl_uint numEventsInWaitList, const cl_event* waitList, cl_event* event, cl_int* errcode_ret)
{
	if(!hostReadable && (mapFlags & CL_MAP_READ))
		return returnError<void*>(CL_INVALID_OPERATION, errcode_ret, __FILE__, __LINE__, "Cannot read from not host-readable image!");
	if(!hostWriteable && ((mapFlags & CL_MAP_WRITE) || (mapFlags & CL_MAP_WRITE_INVALIDATE_REGION)))
		return returnError<void*>(CL_INVALID_OPERATION, errcode_ret, __FILE__, __LINE__, "Cannot write to not host-writeable image!");

	if(rowPitchOutput == nullptr)
		return returnError<void*>(CL_INVALID_VALUE, errcode_ret, __FILE__, __LINE__, "Image row-pitch output pointer is NULL!");
	if((imageType.isImageArray || imageType.numDimensions > 2) && slicePitchOutput == nullptr)
		return returnError<void*>(CL_INVALID_VALUE, errcode_ret, __FILE__, __LINE__, "Image slice-pitch output pointer is NULL!");
...
TextureConfiguration Image::toTextureConfiguration() const
{
	...
	BasicTextureSetup basicSetup(reinterpret_cast<uintptr_t>(deviceBuffer->qpuPointer) / 4096, textureType);
...
{
	if((origin[2] != 0 && (imageType.numDimensions + imageType.isImageArray) < 3) || (origin[1] != 0 && (imageType.numDimensions + imageType.isImageArray) < 2))
	...
	if((origin[2] != 1 && (imageType.numDimensions + imageType.isImageArray) < 3) || (origin[1] != 1 && (imageType.numDimensions + imageType.isImageArray) < 2))
	...
	if(channelOrder.id == CHANNEL_Y_U_Y_V.id)
	{
		...
		if(origin[0] % 2 != 0 || region[0] % 2 != 0)
			return returnError(CL_INVALID_VALUE, __FILE__, __LINE__, "For YUV-images, the origin x-coordinate as well as width of the area accessed need to be even pixels!");
	}
	return CL_SUCCESS;
}
...
ImageAccess::ImageAccess(Image* image, void* hostPtr, bool writeImage, const std::size_t origin[3], const std::size_t region[3]) : image(image), hostPointer(hostPtr), writeToImage(writeImage), hostRowPitch(0), hostSlicePitch(0)
{
...
ImageCopy::ImageCopy(Image* src, Image* dst, const std::size_t srcOrigin[3], const std::size_t dstOrigin[3], const std::size_t region[3]) : source(src), destination(dst)
{
...
{
	if(TextureAccessor::copyPixelData(*source->accessor.get(), *destination->accessor.get(), sourceOrigin, destOrigin, region))
		return CL_SUCCESS;
	else
		return CL_INVALID_OPERATION;
}

ImageFill::ImageFill(Image* img, const void* color, const std::size_t origin[3], const std::size_t region[3]) : image(img)
{
...
ImageCopyBuffer::ImageCopyBuffer(Image* image, Buffer* buffer, bool copyIntoImage, const std::size_t imgOrigin[3], const std::size_t region[3], const size_t bufferOffset) : image(image), buffer(buffer), copyIntoImage(copyIntoImage), bufferOffset(bufferOffset)
{
...
cl_int ImageCopyBuffer::operator()(Event* event)
{
	...
	//OpenCL 1.2 specifies the region copied to be width [* height] [* depth], therefore the pitches match the sizes (OpenCL 1.2 specification, page 111)
	...
	if(copyIntoImage)
		image->accessor->writePixelData(imageOrigin, imageRegion, reinterpret_cast<void*>(hostPtr), rowPitch, slicePitch);
	else
		image->accessor->readPixelData(imageOrigin, imageRegion, reinterpret_cast<void*>(hostPtr), rowPitch, slicePitch);

	return CL_SUCCESS;
}

ImageMapping::ImageMapping(Image* image, void* hostPtr, bool unmap, const std::size_t origin[3], const std::size_t region[3]) : BufferMapping(image, hostPtr, unmap)
{
	memcpy(this->origin.data(), origin, 3 * sizeof(size_t));
	memcpy(this->region.data(), region, 3 * sizeof(size_t));
}
...
cl_mem VC4CL_FUNC(clCreateImage)(cl_context context, cl_mem_flags flags, const cl_image_format* image_format, const cl_image_desc* image_desc, void* host_ptr, cl_int* errcode_ret)
{
	...
	if(host_ptr == nullptr && (image_desc->image_row_pitch != 0 || image_desc->image_slice_pitch != 0))
		return returnError<cl_mem>(CL_INVALID_IMAGE_DESCRIPTOR, errcode_ret, __FILE__, __LINE__, "Image row and slice pitches need to be zero, if no host-pointer is set!");
	if(image_desc->num_mip_levels != 0 || image_desc->num_samples != 0)
		return returnError<cl_mem>(CL_INVALID_IMAGE_DESCRIPTOR, errcode_ret, __FILE__, __LINE__, "Mip-map levels and sampler are not supported!");
	if((image_desc->image_type == CL_MEM_OBJECT_IMAGE1D_BUFFER) != (image_desc->buffer != nullptr))
		return returnError<cl_mem>(CL_INVALID_IMAGE_DESCRIPTOR, errcode_ret, __FILE__, __LINE__, "Buffer must be set if, and only if, the image-type is CL_MEM_OBJECT_IMAGE1D_BUFFER!");
	...
	/*
	 * "For CL_MEM_OBJECT_IMAGE1D_BUFFER image type, if the CL_MEM_READ_WRITE, CL_MEM_READ_ONLY or CL_MEM_WRITE_ONLY values are not specified in flags,
	 * they are inherited from the corresponding memory access qualifers associated with buffer.The	CL_MEM_USE_HOST_PTR , CL_MEM_ALLOC_HOST_PTR and CL_MEM_COPY_HOST_PTR
	 * 	values cannot be specified in flags but are inherited from the corresponding memory access qualifiers associated with buffer. [...] If the CL_MEM_HOST_WRITE_ONLY,
	 * 	CL_MEM_HOST_READ_ONLY or CL_MEM_HOST_NO_ACCESS values are not specified in flags, they are inherited from the corresponding memory access qualifiers associated with buffer."
	 */
 ...
 *
 *  \param command_queue refers to the command-queue in which the read / write command will be queued. command_queue and image must be created with the same OpenCL context.
 *
 ...
 *  If blocking_read is CL_FALSE i.e. the read command is non-blocking, clEnqueueReadImage queues a non-blocking read command and returns. The contents of the buffer that ptr points to cannot be used until the read command has completed.
 *  The event argument returns an event object which can be used to query the execution status of the read command. When the read command has completed,
 *  the contents of the buffer that ptr points to can be used by the application.
 *
 *  \param origin defines the (x, y, z) offset in pixels in the 1D, 2D or 3D image, the (x, y) offset and the image index in the 2D image array or the (x) offset and the image index in the 1D image array.
 *  If image is a 2D image object, origin[2] must be 0. If image is a 1D image or 1D image buffer object, origin[1] and origin[2] must be 0. If image is a 1D image array object, origin[2] must be 0.
 *  If image is a 1D image array object, origin[1] describes the image index in the 1D image array. If image is a 2D image array object, origin[2] describes the image index in the 2D image array.
 *
 *  \param region defines the (width, height, depth) in pixels of the 1D, 2D or 3D rectangle, the (width, height) in pixels of the 2D rectangle and the number of images of a 2D image array or the (width) in pixels of the 1D rectangle and the number of images of a 1D image array.
 *  If image is a 2D image object, region[2] must be 1. If image is a 1D image or 1D image buffer object, region[1] and region[2] must be 1. If image is a 1D image array object, region[2] must be 1.
 *  The values in region cannot be 0.
 *
 *  \param row_pitch in clEnqueueReadImage and input_row_pitch in clEnqueueWriteImage is the length of each row in bytes.
 *  This value must be greater than or equal to the element size in bytes * width. If row_pitch (or input_row_pitch) is set to 0, the appropriate row pitch is calculated based on the size of each element in bytes multiplied by width.
 *
 *  \param slice_pitch in clEnqueueReadImage and input_slice_pitch in clEnqueueWriteImage is the size in bytes of the 2D slice of the 3D region of a 3D image or each image of a 1D or 2D image array being read or written respectively.
 *  This must be 0 if image is a 1D or 2D image. This value must be greater than or equal to row_pitch * height. If slice_pitch (or input_slice_pitch) is set to 0, the appropriate slice pitch is calculated based on the row_pitch * height.
 *
 ...
 *
 *  \return clEnqueueReadImage and clEnqueueWriteImage return CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 ...
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  NOTE: Calling clEnqueueReadImage to read a region of the image with the ptr argument value set to host_ptr + (origin[2] * image slice pitch + origin[1] * image row pitch + origin[0] * bytes per pixel),
 *  where host_ptr is a pointer to the memory region specified when the image being read is created with CL_MEM_USE_HOST_PTR , must meet the following requirements in order to avoid undefined behavior:
...
cl_int VC4CL_FUNC(clEnqueueReadImage)(cl_command_queue command_queue, cl_mem image, cl_bool blocking_read, const size_t* origin, const size_t* region, size_t row_pitch, size_t slice_pitch, void* ptr, cl_uint num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event)
{
	...
	return toType<Image>(image)->enqueueRead(toType<CommandQueue>(command_queue), blocking_read, origin, region, row_pitch, slice_pitch, ptr, num_events_in_wait_list, event_wait_list, event);
}
 ...
 *  The memory pointed to by ptr can be reused by the application after the clEnqueueWriteImage call returns. If blocking_write is CL_FALSE , the OpenCL implementation will use ptr to perform a non-blocking write.
 *  As the write is non-blocking the implementation can return immediately. The memory pointed to by ptr cannot be reused by the application after the call returns.
 ...
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  NOTE: Calling clEnqueueWriteImage to update the latest bits in a region of the image with the ptr argument value set to host_ptr + (origin[2] * image slice pitch + origin[1] * image row pitch + origin[0] * bytes per pixel),
 *  where host_ptr is a pointer to the memory region specified when the image being written is created with CL_MEM_USE_HOST_PTR , must meet the following requirements in order to avoid undefined behavior:
...
cl_int VC4CL_FUNC(clEnqueueWriteImage)(cl_command_queue command_queue, cl_mem image, cl_bool blocking_write, const size_t* origin, const size_t* region, size_t input_row_pitch, size_t input_slice_pitch, const void* ptr, cl_uint num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event)
{
	...
	return toType<Image>(image)->enqueueWrite(toType<CommandQueue>(command_queue), blocking_write, origin, region, input_row_pitch, input_slice_pitch, ptr, num_events_in_wait_list, event_wait_list, event);
}
 ...
 *
 *  \param command_queue refers to the command-queue in which the copy command will be queued. The OpenCL context associated with command_queue, src_image and dst_image must be the same.
 *
 *  \param src_origin defines the (x, y, z) offset in pixels in the 1D, 2D or 3D image, the (x, y) offset and the image index in the 2D image array or the (x) offset and the image index in the 1D image array.
 *  If image is a 2D image object, src_origin[2] must be 0. If src_image is a 1D image object, src_origin[1] and src_origin[2] must be 0. If src_image is a 1D image array object, src_origin[2] must be 0.
 *  If src_image is a 1D image array object, src_origin[1] describes the image index in the 1D image array. If src_image is a 2D image array object, src_origin[2] describes the image index in the 2D image array.
 *
 *  \param dst_origin defines the (x, y, z) offset in pixels in the 1D, 2D or 3D image, the (x, y) offset and the image index in the 2D image array or the (x) offset and the image index in the 1D image array.
 *  If dst_image is a 2D image object, dst_origin[2] must be 0. If dst_image is a 1D image or 1D image buffer object, dst_origin[1] and dst_origin[2] must be 0. If dst_image is a 1D image array object, dst_origin[2] must be 0.
 *  If dst_image is a 1D image array object, dst_origin[1] describes the image index in the 1D image array. If dst_image is a 2D image array object, dst_origin[2] describes the image index in the 2D image array.
 *
 *  \param region defines the (width, height, depth) in pixels of the 1D, 2D or 3D rectangle, the (width, height) in pixels of the 2D rectangle and the number of images of a 2D image array or the (width) in pixels of the 1D rectangle and the number of images of a 1D image array.
 *  If src_image or dst_image is a 2D image object, region[2] must be 1. If src_image or dst_image is a 1D image or 1D image buffer object, region[1] and region[2] must be 1.
 *  If src_image or dst_image is a 1D image array object, region[2] must be 1. The values in region cannot be 0.
 *
 ...
 *  If event_wait_list is not NULL , the list of events pointed to by event_wait_list must be valid and num_events_in_wait_list must be greater than 0. The events specified in event_wait_list act as synchronization points.
 ...
 *
 *  \return clEnqueueCopyImage returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
...
cl_int VC4CL_FUNC(clEnqueueCopyImage)(cl_command_queue command_queue, cl_mem src_image, cl_mem dst_image, const size_t* src_origin, const size_t* dst_origin, const size_t* region, cl_uint num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event)
{
	...
	return toType<Image>(src_image)->enqueueCopyInto(toType<CommandQueue>(command_queue), toType<Image>(dst_image), src_origin, dst_origin, region, num_events_in_wait_list, event_wait_list, event);
}

/*!
 * OpenCL 1.2 specification, pages 106+:
 *
 *  Enqueues a command to fill an image object with a specified color. The usage information which indicates whether the memory object can be read or written by a kernel and/or the host and is given by the cl_mem_flags argument value
 *  specified when image is created is ignored by clEnqueueFillImage.
 *
 *  \param command_queue refers to the command-queue in which the fill command will be queued. The OpenCL context associated with command_queue and image must be the same.
 *
 *  \param image is a valid image object.
 *
 *  \param fill_color is the fill color. The fill color is a four component RGBA floating-point color value if the image channel data type is not an unnormalized signed and unsigned integer type,
 *  is a four component signed integer value if the image channel data type is an unnormalized signed integer type and is a four component unsigned integer value if the image channel data type is an unnormalized unsigned integer type.
 ...
 *  If the event_wait_list and the event arguments are not NULL , the event argument should not refer to an element of the event_wait_list array.
 *
 *  \return clEnqueueFillImage returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
 *  - CL_INVALID_CONTEXT if the context associated with command_queue and image are not the same or if the context associated with command_queue and events in event_wait_list are not the same.
 *  - CL_INVALID_MEM_OBJECT if image is not a valid image object.
...
cl_int VC4CL_FUNC(clEnqueueFillImage)(cl_command_queue command_queue, cl_mem image, const void* fill_color, const size_t* origin, const size_t* region, cl_uint num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event)
{
	...
	return toType<Image>(image)->enqueueFill(toType<CommandQueue>(command_queue), fill_color, origin, region, num_events_in_wait_list, event_wait_list, event);
}
 ...
 *
 *  \param command_queue must be a valid command-queue. The OpenCL context associated with command_queue, src_image and dst_buffer must be the same.
 *
 ...
 *
 *  \param src_origin defines the (x, y, z) offset in pixels in the 1D, 2D or 3D image, the (x, y) offset and the image index in the 2D image array or the (x) offset and the image index in the 1D image array.
 *  If src_image is a 2D image object, src_origin[2] must be 0. If src_image is a 1D image or 1D image buffer object, src_origin[1] and src_origin[2] must be 0. If src_image is a 1D image array object,
 *  src_origin[2] must be 0. If src_image is a 1D image array object, src_origin[1] describes the image index in the 1D image array. If src_image is a 2D image array object, src_origin[2] describes the image index in the 2D image array.
 *
 *  \param region defines the (width, height, depth) in pixels of the 1D, 2D or 3D rectangle, the (width, height) in pixels of the 2D rectangle and the number of images of a 2D image array or the (width) in pixels of the 1D rectangle and the number of images of a 1D image array.
 *  If src_image is a 2D image object, region[2] must be 1. If src_image is a 1D image or 1D image buffer object, region[1] and region[2] must be 1. If src_image is a 1D image array object, region[2] must be 1.
 *  The values in region cannot be 0.
 *
 *  \param dst_offset refers to the offset where to begin copying data into dst_buffer. The size in bytes of the region to be copied referred to as dst_cb is computed as width * height * depth * bytes/image element if src_image is a 3D image object,
 *  is computed as width * height * bytes/image element if src_image is a 2D image, is computed as width * height * arraysize * bytes/image element if src_image is a 2D image array object,
 *  is computed as width * bytes/image element if src_image is a 1D image or 1D image buffer object and is computed as width * arraysize * bytes/image element if src_image is a 1D image array object.
 *
 *  \param event_wait_list and num_events_in_wait_list specify events that need to complete before this particular command can be executed. If event_wait_list is NULL , then this particular command does not wait on any event to complete.
 *  If event_wait_list is NULL , num_events_in_wait_list must be 0. If event_wait_list is not NULL , the list of events pointed to by event_wait_list must be valid and num_events_in_wait_list must be greater than 0.
 *  The events specified in event_wait_list act as synchronization points. The context associated with events in event_wait_list and command_queue must be the same. The memory associated with event_wait_list can be reused or freed after the function returns.
 ...
 *  If the event_wait_list and the event arguments are not NULL , the event argument should not refer to an element of the event_wait_list array.
 *
 *  \return clEnqueueCopyImageToBuffer returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
...
cl_int VC4CL_FUNC(clEnqueueCopyImageToBuffer)(cl_command_queue command_queue, cl_mem src_image, cl_mem dst_buffer, const size_t* src_origin, const size_t* region, size_t dst_offset, cl_uint num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event)
{
	...
	return toType<Image>(src_image)->enqueueCopyFromToBuffer(toType<CommandQueue>(command_queue), toType<Buffer>(dst_buffer), src_origin, region, dst_offset, false, num_events_in_wait_list, event_wait_list, event);
}
 ...
 *
 *  \param command_queue must be a valid command-queue. The OpenCL context associated with command_queue, src_buffer and dst_image must be the same.
 *
 ...
 *
 *  \param dst_origin defines the (x, y, z) offset in pixels in the 1D, 2D or 3D image, the (x, y) offset and the image index in the 2D image array or the (x) offset and the image index in the 1D image array.
 *  If dst_image is a 2D image object, dst_origin[2] must be 0. If dst_image is a 1D image or 1D image buffer object, dst_origin[1] and dst_origin[2] must be 0. If dst_image is a 1D image array object,
 *  dst_origin[2] must be 0. If dst_image is a 1D image array object, dst_origin[1] describes the image index in the 1D image array. If dst_image is a 2D image array object, dst_origin[2] describes the image index in the 2D image array.
 *
 *  \param region defines the (width, height, depth) in pixels of the 1D, 2D or 3D rectangle, the (width, height) in pixels of the 2D rectangle and the number of images of a 2D image array or the (width) in pixels of the 1D rectangle and the number of images of a 1D image array.
 *  If dst_image is a 2D image object, region[2] must be 1. If dst_image is a 1D image or 1D image buffer object, region[1] and region[2] must be 1. If dst_image is a 1D image array object, region[2] must be 1. The values in region cannot be 0.
 *
 *  The size in bytes of the region to be copied from src_buffer referred to as src_cb is computed as width * height * depth * bytes/image element if dst_image is a 3D image object, is computed as width * height * bytes/image element if dst_image is a 2D image,
 *  is computed as width * height * arraysize * bytes/image element if dst_image is a 2D image array object, is computed as width * bytes/image element if dst_image is a 1D image or 1D image buffer object and is computed as
 ...
 *  If the event_wait_list and the event arguments are not NULL , the event argument should not refer to an element of the event_wait_list array.
 *
 *  \return clEnqueueCopyBufferToImage returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
...
cl_int VC4CL_FUNC(clEnqueueCopyBufferToImage)(cl_command_queue command_queue, cl_mem src_buffer, cl_mem dst_image, size_t src_offset, const size_t* dst_origin, const size_t* region, cl_uint num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event)
{
	...
	return toType<Image>(dst_image)->enqueueCopyFromToBuffer(toType<CommandQueue>(command_queue), toType<Buffer>(src_buffer), dst_origin, region, src_offset, true, num_events_in_wait_list, event_wait_list, event);
}
 ...
 *  If blocking_map is CL_FALSE i.e. map operation is non-blocking, the pointer to the mapped region returned by clEnqueueMapImage cannot be used until the map command has completed.
 *  The event argument returns an event object which can be used to query the execution status of the map command. When the map command is completed, the application can access the contents of the mapped region using the pointer returned by clEnqueueMapImage.
 *
 ...
 *
 *  \param image_slice_pitch returns the size in bytes of each 2D slice of a 3D image or the size of each 1D or 2D image in a 1D or 2D image array for the mapped region. For a 1D and 2D image,
 *  zero is returned if this argument is not NULL . For a 3D image, 1D and 2D image array, image_slice_pitch must be a non- NULL value.
 *
 *  \param event_wait_list and num_events_in_wait_list specify events that need to complete before clEnqueueMapImage can be executed. If event_wait_list is NULL , then clEnqueueMapImage does not wait on any event to complete.
 *  If event_wait_list is NULL , num_events_in_wait_list must be 0. If event_wait_list is not NULL , the list of events pointed to by event_wait_list must be valid and num_events_in_wait_list must be greater than 0.
 *  The events specified in event_wait_list act as synchronization points. The context associated with events in event_wait_list and command_queue must be the same.
 *  The memory associated with event_wait_list can be reused or freed after the function returns.
 *
 ...
 *  \param errcode_ret will return an appropriate error code. If errcode_ret is NULL , no error code is returned.
 *
 *  \return clEnqueueMapImage will return a pointer to the mapped region. The errcode_ret is set to CL_SUCCESS. A NULL pointer is returned otherwise with one of the following error values returned in errcode_ret:
 *  - CL_INVALID_COMMAND_QUEUE if command_queue is not a valid command-queue.
 ...
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  NOTE: The pointer returned maps a 1D, 2D or 3D region starting at origin and is at least region[0] pixels in size for a 1D image, 1D image buffer or 1D image array, (image_row_pitch * region[1]) pixels in size for a 2D image or 2D image array,
 *  and (image_slice_pitch * region[2]) pixels in size for a 3D image. The result of a memory access outside this region is undefined. If the image object is created with CL_MEM_USE_HOST_PTR set in mem_flags,
...
void* VC4CL_FUNC(clEnqueueMapImage)(cl_command_queue command_queue, cl_mem image, cl_bool blocking_map, cl_map_flags map_flags, const size_t* origin, const size_t* region, size_t* image_row_pitch, size_t* image_slice_pitch, cl_uint num_events_in_wait_list, const cl_event* event_wait_list, cl_event* event, cl_int* errcode_ret)
{
	...
	return toType<Image>(image)->enqueueMap(toType<CommandQueue>(command_queue), blocking_map, map_flags, origin, region, image_row_pitch, image_slice_pitch, num_events_in_wait_list, event_wait_list, event, errcode_ret);
}
 ...
 *
 *  \param param_name specifies the information to query. The list of supported param_name types and the information returned in param_value by clGetImageInfo is described in table 5.9.
 *
 ...
 *
 *  \param param_value_size is used to specify the size in bytes of memory pointed to by param_value. This size must be >= size of return type as described in table 5.9.
 ...
 *
 *  \return clGetImageInfo returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
