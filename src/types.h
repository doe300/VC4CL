...
#if use_cl_khr_icd
		static_assert(offsetof(_cl_platform_id, dispatch) == 0, "The ICD dispatch-object is required to have no offset!");
...
#if use_cl_khr_icd
		static_assert(offsetof(_cl_command_queue, dispatch) == 0, "The ICD dispatch-object is required to have no offset!");
		assert(dispatch != nullptr);
#endif
		assert(object != nullptr);
	}
 ...
 *
 *  An event object can be used to track the execution status of a command. The API calls that enqueue commands to a command-queue create a new event object that is returned in the event
 *  argument. In case of an error enqueuing the command in the command-queue the event argument does not return an event object.
 ...
 *  - CL_RUNNING – This indicates that the device has started executing this command. In order for the execution status of an enqueued command to change from CL_SUBMITTED
 *  to CL_RUNNING , all events that this command is waiting on must have completed successfully i.e. their execution status must be CL_COMPLETE .
 ...
 *  The program executable can be generated online or offline by the OpenCL compiler for the appropriate target device(s).
 *
...
#if use_cl_khr_icd
		static_assert(offsetof(_cl_counter_vc4cl, dispatch) == 0, "The ICD dispatch-object is required to have no offset!");
		assert(dispatch != nullptr);
#endif
		assert(object != nullptr);
	}
