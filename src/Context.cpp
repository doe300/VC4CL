 ...
 *  Creates an OpenCL context.  An OpenCL context is created with one or more devices. Contexts are used by the OpenCL runtime for managing objects such as command-queues, memory,
 *  program and kernel objects and for executing kernels on one or more devices specified in the context.
 *
 *  \param properties specifies a list of context property names and their corresponding values.  Each property name is immediately followed by the corresponding desired value.
 *  The list is terminated with 0. The list of supported properties is described in table 4.5. properties can be NULL in which case the platform that is selected is implementation-defined.
 *
 *  \param num_devices is the number of devices specified in the devices argument.
 *
 *  \param devices is a pointer to a list of unique devices returned by clGetDeviceIDs or sub-devices created by clCreateSubDevices for a platform.
 *
 *  \param pfn_notify is a callback function that can be registered by the application.  This callback function will be used by the OpenCL implementation to report information on errors
 *  during context creation as well as errors that occur at runtime in this context.  This callback function may be called asynchronously by the OpenCL implementation.
 ...
 *
 *  \return clCreateContext returns a valid non-zero context and errcode_ret is set to CL_SUCCESS if the context is created successfully.
 ...
 *  - CL_INVALID_PROPERTY if context property name in properties is not a supported property name, if the value specified for a supported property name is not valid,
 ...
 */
cl_context VC4CL_FUNC(clCreateContext)(const cl_context_properties* properties, cl_uint num_devices, const cl_device_id* devices, void(CL_CALLBACK* pfn_notify)(const char* errinfo, const void *private_info, size_t cb, void* user_data), void* user_data, cl_int* errcode_ret)
{
	...
	if(properties != nullptr)
	{
		...
		while(*ptr != 0)
		{
			...
			else if(*ptr == CL_CONTEXT_INTEROP_USER_SYNC)
			{
				...
				return returnError<cl_context>(CL_INVALID_PROPERTY, errcode_ret, __FILE__, __LINE__, buildString("Invalid cl_context_properties value %d!", *ptr));
		}
	}
	...
	for(; i < num_devices; ++i)
	{
		...
		if(devices[i] != Platform::getVC4CLPlatform().VideoCoreIVGPU.toBase())
			return returnError<cl_context>(CL_INVALID_DEVICE, errcode_ret, __FILE__, __LINE__, "Device is not the VC4CL GPU device!");
	}
 ...
 *
 *  \param properties specifies a list of context property names and their corresponding values.  Each property name is immediately followed by the corresponding desired value.
 *  The list of supported properties is described in table 4.5. properties can also be NULL in which case the platform that is selected is implementation-defined.
 *
 ...
 *  \param errcode_ret will return an appropriate error code.  If errcode_ret is NULL, no error code is returned.
 *
 *  \return clCreateContextFromType returns a valid non-zero context and errcode_ret is set to CL_SUCCESS if the context is created successfully.
 *  Otherwise, it returns a NULL value with the following error values returned in errcode_ret:
 *  - CL_INVALID_PLATFORM if properties is NULL and no platform could be selected or if platform value specified in properties is not a valid platform.
 *  - CL_INVALID_PROPERTY if context property name in properties is not a supported property name, if the value specified for a supported property name is not valid,
 *  or if the same property name is specified more than once.
 ...
 */
cl_context VC4CL_FUNC(clCreateContextFromType)(const cl_context_properties* properties, cl_device_type device_type, void (CL_CALLBACK* pfn_notify)(const char* errinfo, const void* private_info, size_t cb, void* user_data), void* user_data, cl_int* errcode_ret)
{
	cl_uint num_devices = 0;
	cl_device_id device;
	cl_uint error = VC4CL_FUNC(clGetDeviceIDs)(Platform::getVC4CLPlatform().toBase(), device_type, 1, &device, &num_devices);
	if(error != CL_SUCCESS)
 ...
 *
 *  \return clRetainContext returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_CONTEXT if context is not a valid OpenCL context.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  clCreateContext and clCreateContextFromType perform an implicit retain. This is very helpful for 3rdparty libraries, which typically get a context passed to them by the application.
 *  However, it is possible that the application may delete the context without informing the library. Allowing functions to attach to (i.e. retain) and release a context solves the problem of a context
 ...
 *
 *  \return clReleaseContext returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_CONTEXT if context is not a valid OpenCL context.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resource s required by the OpenCL implementation on the host.
 *
 ...
 *
 *  \param param_value_size specifies the size in bytes of memory pointed to by param_value.  This size must be greater than or equal to the size of return type as described in table 4.6.
 *
 ...
 *
 *  The list of supported param_name values and the information returned in param_value by clGetContextInfois described in table 4.6.
 *
 *  \return clGetContextInfo returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_CONTEXT if context is not a valid context.
 *  - CL_INVALID_VALUE if param_name is not one of the supported values or if size in bytes specified by param_value_size is < size of return type as specified in table 4.6
 *  and param_value is not a NULL value.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
cl_int VC4CL_FUNC(clGetContextInfo)(cl_context context, cl_context_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret)
{
	CHECK_CONTEXT(toType<Context>(context))
