...
cl_int Event::setUserEventStatus(cl_int execution_status)
{
	if(execution_status != CL_COMPLETE && execution_status >= 0)
		return returnError(CL_INVALID_VALUE, __FILE__, __LINE__, buildString("Event has already finished with status %d", execution_status));
...
cl_int Event::getInfo(cl_event_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret) const
{
	switch(param_name)
	{
		...
		case CL_EVENT_COMMAND_TYPE:
			return returnValue<cl_command_type>(static_cast<cl_command_type>(type), param_value_size, param_value, param_value_size_ret);
...
cl_int Event::getProfilingInfo(cl_profiling_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret) const
{
	...
	return returnError(CL_INVALID_VALUE, __FILE__, __LINE__, buildString("Invalid cl_profiling_info value %d", param_name));
}
...
cl_int Event::waitFor() const
{
	...
	for(Event* e : waitList)
	{
		if(e->waitFor() != CL_SUCCESS)
			return returnError(CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST, __FILE__, __LINE__, "Error in event in wait-list");
	}
 ...
 *
 *  \retrun clCreateUserEvent returns a valid non-zero event object and errcode_ret is set to CL_SUCCESS if the user event object is created successfully.
 ...
 *
 *  \param execution_status specifies the new execution status to be set and can be CL_COMPLETE or a negative integer value to indicate an error.
 *  A negative integer value causes all enqueued commands that wait on this user event to be terminated. clSetUserEventStatus can only be called once to change the execution status of event.
 *
 *  \return clSetUserEventStatus returns CL_SUCCESS if the function was executed successfully. Otherwise, it returns one of the following errors:
 ...
 *
 *  \return clWaitForEvents returns CL_SUCCESS if the execution status of all events in event_list is CL_COMPLETE . Otherwise, it returns one of the following errors:
...
cl_int VC4CL_FUNC(clEnqueueWaitForEvents)(cl_command_queue command_queue, cl_uint num_events, const cl_event *event_list)
{
 ...
 *
 *  \param param_name specifies the information to query. The list of supported param_name types and the information returned in param_value by clGetEventInfo is described in table 5.18.
 *
 ...
 *
 *  \param param_value_size is used to specify the size in bytes of memory pointed to by param_value. This size must be >= size of return type as described in table 5.18.
 *
 ...
 *
 *  \return clGetEventInfo returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 ...
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
cl_int VC4CL_FUNC(clGetEventInfo)(cl_event event, cl_event_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret)
{
	CHECK_EVENT(toType<Event>(event))
 ...
 *
 *  Registers a user callback function for a specific command execution status. The registered callback function will be called when the execution status of command associated with event
 *  changes to an execution status equal to or past the status specified by command_exec_status. Each call to clSetEventCallback registers the specified user callback function on a callback
 ...
 *
 *  \param command_exec_callback_type specifies the command execution status for which the callback is registered. The command execution callback values for which a callback can be registered are:
 *  CL_SUBMITTED , CL_RUNNING or CL_COMPLETE . There is no guarantee that the callback functions registered for various execution status values for an event will be called in the exact order that the execution status of a command changes.
 ...
 *
 *  \param pfn_event_notify is the event callback function that can be registered by the application. This callback function may be called asynchronously by the OpenCL implementation.
 ...
 *   - event_command_exec_status represents the execution status of command for which this callback function is invoked. Refer to table 5.18 for the command execution status values.
 ...
 *
 *  All callbacks registered for an event object must be called. All enqueued callbacks shall be called before the event object is destroyed. Callbacks must return promptly.
 ...
 *
 *  If an application needs to wait for completion of a routine from the above list in a callback, please use the non-blocking form of the function, and assign a completion callback to it to do the
 *  remainder of your work. Note that when a callback (or other code) enqueues commands to a command-queue, the commands are not required to begin execution until the queue is flushed.
 *  In standard usage, blocking enqueue calls serve this role by implicitly flushing the queue. Since blocking calls are not permitted in callbacks, those callbacks that enqueue commands on a
 *  command queue should either call clFlush on the queue before returning or arrange for clFlush to be called later on another thread.
 *
 *  \return clSetEventCallback returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 ...
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
cl_int VC4CL_FUNC(clSetEventCallback)(cl_event event, cl_int command_exec_callback_type, void(CL_CALLBACK* pfn_event_notify)(cl_event event, cl_int event_command_exec_status, void* user_data), void* user_data)
{
	CHECK_EVENT(toType<Event>(event))
 ...
 *
 *  \return clRetainEvent returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_EVENT if event is not a valid event object.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
 ...
 *
 *  \return clReleaseEvent returns CL_SUCCESS if the function is executed successfully. Otherwise, it returns one of the following errors:
 *  - CL_INVALID_EVENT if event is not a valid event object.
 *  - CL_OUT_OF_RESOURCES if there is a failure to allocate resources required by the OpenCL implementation on the device.
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 *
 *  The event object is deleted once the reference count becomes zero, the specific command identified by this event has completed (or terminated) and there are no commands in the
 *  command-queues of a context that require a wait for this event to complete.
 *
 *  NOTE: Developers should be careful when releasing their last reference count on events created by clCreateUserEvent that have not yet been set to status of CL_COMPLETE or an error.
 *  If the user event was used in the event_wait_list argument passed to a clEnqueue*** API or another application host thread is waiting for it in clWaitForEvents,
 *  those commands and host threads will continue to wait for the event status to reach CL_COMPLETE or error, even after the user has released the object.
 *  Since in this scenario the developer has released his last reference count to the user event, it would be in principle no longer valid for him to change the status of the event
 *  to unblock all the other machinery. As a result the waiting tasks will wait forever, and associated events, cl_mem objects, command queues and contexts are likely to leak.
 ...
 *
 *  \param param_name specifies the profiling data to query. The list of supported param_name types and the information returned in param_value by clGetEventProfilingInfo is described in table 5.19.
 *
 *  \param param_value is a pointer to memory where the appropriate result being queried is returned. If param_value is NULL , it is ignored.
 *
 *  \param param_value_size is used to specify the size in bytes of memory pointed to by param_value. This size must be >= size of return type as described in table 5.19.
 *
 *  \param param_value_size_ret returns the actual size in bytes of data copied to param_value. If param_value_size_ret is NULL , it is ignored.
 *
 *  The unsigned 64-bit values returned can be used to measure the time in nano-seconds consumed by OpenCL commands.
 *
 *  OpenCL devices are required to correctly track time across changes in device frequency and power states. The CL_DEVICE_PROFILING_TIMER_RESOLUTION specifies the resolution of
 *  the timer i.e. the number of nanoseconds elapsed before the timer is incremented.
 *
 *  \return clGetEventProfilingInfo returns CL_SUCCESS if the function is executed successfully and the profiling information has been recorded. Otherwise, it returns one of the following errors:
 ...
 *  - CL_OUT_OF_HOST_MEMORY if there is a failure to allocate resources required by the OpenCL implementation on the host.
 */
cl_int VC4CL_FUNC(clGetEventProfilingInfo)(cl_event event, cl_profiling_info param_name, size_t param_value_size, void* param_value, size_t* param_value_size_ret)
{
	CHECK_EVENT(toType<Event>(event))
